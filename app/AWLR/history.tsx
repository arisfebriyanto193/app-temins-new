...
export default function AnalyticsPage() {
  ...
  const fetchInit = async () => {
    try {
      ...
      const url = `${API_BASE_URL}/api-app/user/awlr/history-mobile.php`;
  ...
  const fetchChartData = async () => {
    setLoadingData(true);
    let url = `${API_DATA_URL}/api/get-data?device_id=${deviceId}&jenis=${selectedSensor}&periode=${period}&mode=ringkas&limit=10`;
      ...
      >
        <View style={[styles.modalContainer, { paddingTop: insets.top }]}>
          ...
          <View style={[styles.modalFooter, { paddingBottom: insets.bottom + 16 }]}>
            ...
            >
              <Ionicons 
                name={selectedFormat === 'excel' ? "logo-microsoft-excel" : "document-text"} 
                size={20} 
                color="white" 
              />
