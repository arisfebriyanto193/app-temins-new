...
function computeForecast(vals: Record<string, SensorValue>): ForecastResult {
  ...
  if (cp > 0.5) return {
    ...
    desc: 'Terpantau presipitasi hujan. Siapkan perlengkapan anti-air jika keluar.',
  };
  ...
  if (rm > 600 && su > 31) return {
    ...
    desc: 'Suhu dan radiasi sangat tinggi. Waspada dehidrasi dan hindari paparan matahari terlalu lama.',
  };
  if (rm > 300 && su > 27) return {
    title: 'Cerah', icon: 'partly-sunny', iconColor: '#f59e0b',
    bgColor: '#fffbeb', accentColor: '#f59e0b',
    desc: 'Cuaca terang dan hangat. Sangat mendukung untuk aktivitas di luar ruangan.',
  };
  return {
    title: 'Cerah Berawan', icon: 'cloudy', iconColor: '#0ea5e9',
...
export default function ForecastScreen() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        const [initRes, cuacaRes] = await Promise.all([
          fetch(`${apiUrl}/api-app/user/aws/history.php`, { headers: { Authorization: `Bearer ${token}` } }),
          fetch(`${apiUrl}/api-app/user/aws/cuaca.php`, { headers: { Authorization: `Bearer ${token}` } }),
        ]);
        const initData = await initRes.json();
        const cuacaData = await cuacaRes.json();
        if (!initData.status) throw new Error('Init gagal');

        setDeviceName(initData.device_name || '');
        setDeviceLocation(`${initData.lokasi || ''}, ${initData.kota || ''}`);

        const sensorMap = new Map(initData.sensors.map((s: any) => [s.code, s]));
        const url = `${apiData}/api/get-data?device_id=${initData.device_id}&jenis=su,ku,rm,cp&mode=raw&limit=100`;
        const dataRes = await fetch(url, { headers: { Authorization: `Bearer ${apiToken}` } });
        const rawData = await dataRes.json();

        let initVals: Record<string, SensorValue> = {};
        if (rawData.status && rawData.data) {
          rawData.data.forEach((item: any) => {
            const code = item.parameter_name;
            if (targetSensors.includes(code) && !initVals[code]) {
              const sInfo = sensorMap.get(code) as any;
              initVals[code] = {
                value: parseFloat(item.value), unit: sInfo?.unit || '',
                label: sInfo?.label || code.toUpperCase(), recorded_at: item.recorded_at,
              };
            }
          });
          setSensorValues(initVals);
          setForecast(computeForecast(initVals));
          const first = Object.values(initVals)[0];
          if (first?.recorded_at) setLastUpdate(first.recorded_at);
        }

        if (cuacaData.status) {
          const client = mqtt.connect('wss://karsacerdasinovatif.web.id:8081', {
            clientId: 'aws_fc_rn_' + Math.random().toString(16).substring(2, 10),
          ...
          client.on('message', (topic: string, payload: Buffer) => {
            ...
            let code = '';
            Object.keys(cuacaData.data || {}).forEach(c => { if (cuacaData.data[c] === topic) code = c; });
            if (code && targetSensors.includes(code)) {
              const now = new Date();
              const pad = (n: number) => n.toString().padStart(2, '0');
              const ts = `${now.getFullYear()}-${pad(now.getMonth()+1)}-${pad(now.getDate())} ${pad(now.getHours())}:${pad(now.getMinutes())}:${pad(now.getSeconds())}`;
              setSensorValues(prev => {
                const next = { ...prev, [code]: { value: val, unit: prev[code]?.unit || '', label: prev[code]?.label || code.toUpperCase(), recorded_at: ts } };
    ...
    fetchData();
    return () => { if (mqttClientRef.current) mqttClientRef.current.end(true); };
  }, []);
        ...
        {!isLoading && (
          <>
            ...
            <View style={styles.metricsGrid}>
              ...
              <MetricCard icon="rainy" label={sensorValues['ch']?.label || 'Curah Hujan'}
...
const styles = StyleSheet.create({
  ...
  locationIcon: {
    backgroundColor: '#e0f2fe', borderRadius: 22, width: 40, height: 40,
  ...
  forecastCard: {
    backgroundColor: '#f0f9ff', borderRadius: 20, borderWidth: 1, borderColor: '#bae6fd',
    padding: 28, alignItems: 'center', gap: 12, minHeight: 240, justifyContent: 'center',
    elevation: 2,
  },
