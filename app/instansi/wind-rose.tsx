...
export default function InstansiWindRoseScreen() {
  ...
  useFocusEffect(
    React.useCallback(() => {
      const load = async () => {
        ...
        setDeviceId(devId);
        try {
          const res = await fetch(`${apiUrl}/api-app/instansi/aws/ds.php?device_id=${devId}`, {
            headers: { Authorization: `Bearer ${token}` },
          });
          const json = await res.json();
          if (json.status && json.selected_device_details) {
            setDeviceName(json.selected_device_details.device?.name || devId);
          }
  ...
  const fetchWind = useCallback(async () => {
    ...
    } catch (e) { console.error('wind fetch error', e); } finally { setLoading(false); }
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false} style={styles.scroll}>
        ...
        <View style={styles.filterRow}>
          <View>
            ...
            <Text style={styles.filterDesc}>Filter noise data angin ketika hujan</Text>
          </View>
        ...
        <View style={styles.statsGrid}>
          {[
            { label: 'Rata-rata', value: `${stats.avg} m/s`, icon: 'speedometer-outline', color: '#3b82f6' },
...
const styles = StyleSheet.create({
  ...
  headerIconWrap: {
    backgroundColor: '#dbeafe', borderRadius: 8, padding: 7,
